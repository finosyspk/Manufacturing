...
exports.getList = async (req, res) => {
  try {
    ...
    let ItemClass = await SeqFunc.getAll(req.sequelizeDB.INV_ItemClass, {}, true, Columns);
...
exports.getOne = async (req, res) => {
  try {
    let ItemClass = await SeqFunc.getOne(req.sequelizeDB.INV_ItemClass, { where:{ItemClassCode: req.query.ItemClassCode} });

    if (ItemClass.success) {
      let AttributeDetail = await SeqFunc.getAll(
        req.sequelizeDB.INV_ItemClassAttributes,
...
exports.delete = async (req, res) => {
  try {
    let ItemClassData = await SeqFunc.getOne(
      req.sequelizeDB.INV_ItemClass,
      { where: { ItemClassCode: req.query.ItemClassCode } },
      Header
    );

    if (ItemClassData.success) {
      await SeqFunc.Delete(req.sequelizeDB.INV_ItemClassAttributes, {
        where: { ItemClassCode: ItemClassData.Data.ItemClassCode },
      });

      await SeqFunc.Delete(req.sequelizeDB.INV_ItemClass, {
        where: { ItemClassCode: ItemClassData.Data.ItemClassCode },
      });
...
exports.CreateOrUpdate = async (req, res) => {
  try {
    ...
    if (ItemClassData.success) {
      await SeqFunc.Delete(req.sequelizeDB.INV_ItemClassAttributes, { where:{ItemClassCode: Header.ItemClassCode} });
      ...
      await SeqFunc.bulkCreate(req.sequelizeDB.INV_ItemClassAttributes,Detail)

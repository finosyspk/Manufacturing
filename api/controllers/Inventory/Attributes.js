...
exports.getList = async (req, res) => {
  try {
    ...
    let Attribute = await SeqFunc.getAll(req.sequelizeDB.INV_AttributeHead, {}, true, Columns);
...
exports.getOne = async (req, res) => {
  try {
    let Attribute = await SeqFunc.getOne(req.sequelizeDB.INV_AttributeHead, { where:{AttHeadCode: req.query.AttHeadCode} });

    if (Attribute.success) {
      let AttributeDetail = await SeqFunc.getAll(
        req.sequelizeDB.INV_AttributeCode,
...
exports.delete = async (req, res) => {
  try {
    let Attributes = await SeqFunc.getOne(
      req.sequelizeDB.INV_AttributeHead,
      {
        where: { AttHeadCode: req.query.AttHeadCode },
      }
    );
    ...
    if (Attributes.success) {
      await SeqFunc.Delete(req.sequelizeDB.INV_AttributeCode, {
        where: { AttHeadCode: req.query.AttHeadCode },
      });
      await SeqFunc.Delete(req.sequelizeDB.INV_AttributeHead, {
        where: { AttHeadCode: req.query.AttHeadCode },
      });
...
exports.CreateOrUpdate = async (req, res) => {
  try {
    ...
    if (AttData.success) {
      await SeqFunc.Delete(req.sequelizeDB.INV_AttributeCode, { where:{AttHeadCode: Header.AttHeadCode} });
      ...
      await SeqFunc.bulkCreate(req.sequelizeDB.INV_AttributeCode,Detail)
